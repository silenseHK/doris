    ...
    $(function () {
        ...
        var App = new Vue({
            ...
            methods:{
                ...
                submit: function(){
                    let [name, desc, label, img_id] = [this.name, this.desc, this.label, parseFloat(this.img_id)]
                    if(!name || !img_id){
                        layer.msg('请将填写必填项')
                        return false;
                    }
                    ...
                    let that = this;
                    $.post("<?= url('college.lecturer/add') ?>",{name,desc,avatar:img_id,label}, function(res){
                        if(res.code == 1){
                            that.img_id = 0;
                            that.img = '';
                            that.name = '';
