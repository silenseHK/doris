    ...
    $(function () {
        ...
        var App = new Vue({
            ...
            data: {
                ...
                title: "<?= $info['title'] ?>",
                sort: <?= $info['sort'] ?>,
                img: "<?= $info['icon']?$info['icon']['file_path']:'' ?>",
                img_id: <?= $info['icon_id'] ?>,
            ...
            methods:{
                ...
                submit: function(){
                    let [title, sort, img_id, status, cate_id] = [this.title, parseInt(this.sort), parseInt(this.img_id), parseInt(this.status), this.cate_id]
                    if(!title || sort<0 || !img_id){
                        layer.msg('请将填写全部选项')
                        return false;
                    }
                    $.post("<?= url('content.onlineQuestions/cateEdit') ?>",{title,img_id,sort,status,cate_id}, function(res){
                        if(res.code == 1){
                            layer.msg('操作成功');
                        }else{
                            layer.msg(res.msg)
